...
use crate::models::{PackageResponse,EnrichedPackage};
...
pub async fn get_all_packages(pool: &sqlx::PgPool) -> Result<Vec<PackageResponse>> {
    // Use raw SQL execution via connection to avoid PREPARE commands entirely
...
pub async fn get_package_by_name(pool: &sqlx::PgPool, name: &str) -> Result<Option<PackageResponse>> {
